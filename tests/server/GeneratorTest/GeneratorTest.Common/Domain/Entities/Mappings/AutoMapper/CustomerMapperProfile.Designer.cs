...
namespace GeneratorTest.Common.Domain.Mappings.AutoMapper
{
  public partial class CustomerMappingProfile : Profile
  {
    ...
    public virtual void SetupMappingToEntity(IMappingExpression<CustomerDTO, Customer> mappingEntity)
    {
      ...
      mappingEntity.ForMember(x => x.Store, map => map.MapFrom((s, t) =>
      {
    ...
    {
      mappingToDto.ForMember(x => x.StoreId, map => map.MapFrom((s, t) => s.Store?.Id));
      this.SetupMappingToDtoExtra(mappingToDto);
    }

    partial void SetupMappingToDtoExtra(IMappingExpression<Customer, CustomerDTO> mappingToDto);
  }
}