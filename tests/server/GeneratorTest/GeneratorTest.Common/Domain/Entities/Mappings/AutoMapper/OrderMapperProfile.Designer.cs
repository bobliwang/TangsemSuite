...
namespace GeneratorTest.Common.Domain.Mappings.AutoMapper
{
  public partial class OrderMappingProfile : Profile
  {
    ...
    public virtual void SetupMappingToEntity(IMappingExpression<OrderDTO, Order> mappingEntity)
    {
      ...
      mappingEntity.ForMember(x => x.Customer, map => map.MapFrom((s, t) =>
      {
      ...
      mappingEntity.ForMember(x => x.Product, map => map.MapFrom((s, t) =>
      {
    ...
    {
      mappingToDto.ForMember(x => x.CustomerId, map => map.MapFrom((s, t) => s.Customer?.CustomerId));
      mappingToDto.ForMember(x => x.ProductId, map => map.MapFrom((s, t) => s.Product?.Id));
      this.SetupMappingToDtoExtra(mappingToDto);
    }

    partial void SetupMappingToDtoExtra(IMappingExpression<Order, OrderDTO> mappingToDto);
  }
}