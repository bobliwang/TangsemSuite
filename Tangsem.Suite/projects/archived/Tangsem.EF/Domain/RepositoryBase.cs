...
namespace Tangsem.EF.Domain
{
  ...
  public abstract class RepositoryBase : IRepository
  {
    ...
    public virtual IDisposable BeginTransaction()
    {
      if (this.IsInTransaction)
      {
        throw new InvalidOperationException("Cannot BeginTransaction because it is already in a transaction.");
      }

      this.Transaction = this.CurrentDbContext.Database.BeginTransaction();

      return new RepositoryTransactionWrapper(this);
    }
    ...
    public IDisposable BeginTransaction(IsolationLevel isolationLevel)
    {
      ...
      this.Transaction = this.CurrentDbContext.Database.BeginTransaction(isolationLevel);
