...
namespace Tangsem.Generator
{
	public class Gen
	{
		...
		private void GenerateForMultipleMetadataTemplates(List<TableMetadata> tableMetadatas)
		{
			...
			template = new Repository_NHibernate_Designer
				{ Configuration = this.GeneratorConfiguration, TableMetadatas = tableMetadatas };
			...
			template = new IRepository_NHibernate_Designer
				{ Configuration = this.GeneratorConfiguration, TableMetadatas = tableMetadatas };
			code = template.TransformText().Trim();
